...
class Animation {
   ...
   */
  constructor(animation: Function, steps: number | null = null) {
...
class AnimatedElement {
  group: string;
  id: string;
  cy: cytoscape.NodeSingular | cytoscape.EdgeSingular | null = null;
   ...
   */
  constructor(animator: Animator, group: string, id: string | null = null) {
   ...
   */
  copy(): AnimatedElement {
    let copy: AnimatedElement = new AnimatedElement(this.animator, this.group, this.id);
    copy.cy = this.cy ? this.cy.clone() : null;
    copy.classes = [...this.classes];
    return copy;
  }

  setCy(cy: cytoscape.NodeSingular | cytoscape.EdgeSingular) {
    this.cy = cy;
  }
}
...
class AnimatedNode extends AnimatedElement {
   ...
   */
  constructor(animator: Animator, label: string, x: number, y: number) {
   ...
   */
  copy(): AnimatedNode {
    let copy: AnimatedNode = super.copy() as AnimatedNode;
    copy.label = this.label;
    copy.position = { x: this.position.x, y: this.position.y };
    return copy;
  }

  setCy(cy: cytoscape.NodeSingular) {
    this.cy = cy;
    cy.on('select', () => {
...
class AnimatedEdge extends AnimatedElement {
   ...
   */
  constructor(animator: Animator, id: string, source: string, target: string) {
   ...
   */
  copy(): AnimatedEdge {
    let copy: AnimatedEdge = super.copy() as AnimatedEdge;
...
class AnimatorEventBus {
  /**
   * The events dictionary maps event names to an array of listeners.
   */
  events: {
   ...
   * @param listener the listener function
   */
  off(eventName: string, listener: Function | null = null) {
    if (!this.eventExists(eventName)) {
      return;
    }
    if (listener === null) {
 ...
 */
class State {
  elements: Array<AnimatedElement>;
  cy: Object;
  constructor(elements: Array<AnimatedElement>, cy: Object) {
...
class Snapshot {
   ...
   */
  constructor(state: State, historyIndex: number, name: String | null) {
...
class Animator {
  cy: cytoscape.Core;
  historyContainer: cytoscape.Core;
  queue: Array<Animation> = [];
  elements: Array<AnimatedElement> = [];
  history: Array<Animation> = [];
  ...
  selectedNode: AnimatedNode | null = null;
  ...
  */
  ...
  step() {
    ...
    if (this.pausedEvent) this.emit(this.pausedEvent);
  ...
  animationFinishedListener(event: any) {
    let delay = this.animationDelay;
    if (event?.detail?.type !== 'snapshot') {
  ...
  fitToElements(eles: Array<AnimatedElement>) {
   ...
   */
  addNode(label: string, x: number, y: number, nodeData: Object | null = null) {
    // Create a new node and add it to the elements array.
    const node = new AnimatedNode(this, label, x, y);
    const cyNode: cytoscape.ElementDefinition = {
      group: 'nodes',
  ...
  changeNodeColor(id: string, color: string) {
    ...
    const oldColor: string = this.cy.getElementById(id).style('background-color');
    this.queueAnimation(new Animation(nodeUpdate));
    return oldColor;
  }

  changeEdgeColor(id: string, color: string) {
    ...
    const oldColor: string = this.cy.getElementById(id).style('line-color');
    this.queueAnimation(new Animation(edgeUpdate));
    return oldColor;
  }

  addEdge(source: string, target: string, id: string | null = null) {
    ...
    const cyEdge: cytoscape.ElementDefinition = {
      group: 'edges',
  ...
  addEdges(...edges: Array<{ source: string; target: string; id: string | null }>) {
    const addedEdges: Array<AnimatedEdge> = [];
    edges.forEach((edge) => {
      // If id is null, generate a unique id. Otherwise, check if the id
      // already exists. If it already exists, throw an error.
  ...
  moveEdge(id: string, source: string, target: string) {
  ...
  unhighlightCyElements(id: string) {
  ...
  moveNodeRelativeTo(id1: string, id2: string, x: number, y: number) {
  ...
  moveNodes(...nodesToMove: Array<{ id: string; x: number; y: number }>) {
  ...
  moveNodeBy(id: string, dx: number, dy: number) {
    // Get the node and check that it exists and is actually a node.
    const node = this.getNode(id);
    if (!node) {
      throw new Error(`Node with id ${id} does not exist`);
  ...
  moveNodesBy(ids: Array<string>, dx: number, dy: number) {
  ...
  moveAllNodesBy(dx: number, dy: number) {
    this.moveNodesBy(
  ...
  elementHasClass(id: string, className: string): boolean {
    const element = this.elements.find((e) => e.id === id);
  ...
  swapEdges(edge1: AnimatedEdge, edge2: AnimatedEdge) {
  ...
  swapNodes(node1: AnimatedNode, node2: AnimatedNode) {
    const node1Elements = this.getElementsWithClass(node1.id);
    const node1Children: Array<AnimatedNode> = [];
    ...
    const node2Children: Array<AnimatedNode> = [];
    ...
    node1Elements.forEach((node) => {
      const n: AnimatedNode = node as AnimatedNode;
  ...
  annotateNode(id: string, text: string) {
  ...
  takeSnapshot(name: String | null = null) {
    this.queueAnimation(
      new Animation(() => {
  ...
  restoreSnapshot(snapshot: number | Snapshot) {
  ...
  setSelectedNode(node: AnimatedNode | null) {
    this.selectedNode = node;
    document.dispatchEvent(new CustomEvent('selectionChanged'));
  }
}

function copyElement(e: AnimatedElement): AnimatedElement {
  return Object.assign(Object.create(Object.getPrototypeOf(e)), e);
}

class AnimatedTreeNode {
  node: AnimatedNode;
  parent: AnimatedTreeNode | null = null;
  children: Array<AnimatedTreeNode> = [];
  height: number = 0;
  constructor(node: AnimatedNode) {
    this.node = node;
  }

  addChild(child: AnimatedTreeNode) {
    child.parent = this;
    this.children.push(child);
  }

  removeChild(child: AnimatedTreeNode) {
    child.parent = null;
    this.children = this.children.filter((c) => c !== child);
  }

  getRoot(): AnimatedTreeNode {
    let root: AnimatedTreeNode = this;
    while (root.parent) {
      root = root.parent;
    }
    return root;
  ...
  getHeight(): number {
    let height = 0;
    let node: AnimatedTreeNode = this;
...
class AnimatedTree extends AnimatedTreeNode {
  ...
  moveTreeBy(dx: number, dy: number) {
    const nodes = this.getNodes();
